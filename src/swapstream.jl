...
read(s::SwapStream, n::Int) = read(s.io, n)
read(s::SwapStream{true}, x::Type{<:AbstractArray}) = bswap.(read(s.io, x))
read(s::SwapStream{false}, x::Type{<:AbstractArray}) = read(s.io, x)
read(s::SwapStream, x::Type{Int8}) = read(s.io, Int8)


read!(s::SwapStream{true,<:IO}, a::Array{Int8,N}) where N = read!(s.io, a)
read!(s::SwapStream{false,<:IO}, a::Array{Int8,N}) where N = read!(s.io, a)

read!(s::SwapStream{true,<:IO}, a::Array{UInt8,N}) where N = read!(s.io, a)
read!(s::SwapStream{false,<:IO}, a::Array{UInt8,N}) where N = read!(s.io, a)

read!(s::SwapStream{false,IOType}, a::Array{T}) where {IOType<:IO,T} = read!(s.io, a)
read!(s::SwapStream{true,IOType}, a::Array{T}) where {IOType<:IO,T} = bswap.(read!(s.io, a))
...
read!(s::SwapStream{true}, ref::Base.RefValue{<:NTuple{N,T}}) where {N,T} = bswap.(read!(s.io, ref)[])
read!(s::SwapStream{false}, ref::Base.RefValue{<:NTuple{N,T}}) where {N,T} = read!(s.io, ref)[]

write(s::SwapStream{true,<:IO}, x::Array) = write(s.io, mappedarray(ntoh, hton, x))
write(s::SwapStream{true,<:IO}, x::T) where T = write(s.io, bswap.(x))
write(s::SwapStream{false,<:IO}, x::T) where T = write(s.io, x)
